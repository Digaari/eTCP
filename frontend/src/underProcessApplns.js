    ...
    <Container maxWidth="xlg" style={{ paddingBottom: '500px' }}>
      ...
      <TableContainer component={Paper} style={{ marginTop: '20px' }}>
        <Table>
          ...
          <TableBody>
          {formData
              .filter((data) => data.applnStatus === 0 || data.applnStatus === 1) // Filter based on applnStatus FWD OR R.
              .map((data, index) => (
              <TableRow key={index}>
      ...
      <Dialog open={showRejectDialog} onClose={() => setShowRejectDialog(false)}>
        ...
        <DialogTitle>Rejected By:</DialogTitle>
        <DialogContent>
          <TextField
            multiline
            fullWidth
            rows={4}
            variant="outlined"
            value={`${selectedRejectedData?.rejOffrRk} ${selectedRejectedData?.rejOffrName}, ${selectedRejectedData?.rejOffrAppt}, ${selectedRejectedData?.rejBy}`}
          />
        </DialogContent>
        <DialogActions>
...
export default UnderProcessApplns;